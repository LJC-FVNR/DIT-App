    ...
    "DA":{"Initialize": {"call": "session['PHASE']=3;pb_data=PB_Data(**parameters);session['pb_data']=pb_data", 
                         'tab': 'Data', 
                                  ...
                                  "parameters": {"processor": ["Text Processor", "processor", ["immutable", "eval"], 1, "Pre-built Text Processor"],
                                                 ...
                                                 "classification": ["Categorical Features", "list(processor.data.columns)", ["checks", "PB_Data.default_classification"], 4, "Check all categorical columns"],
                                                 "performance": ["Numerical Features", "list(processor.data.columns)", ["checks", "PB_Data.default_performance"], 4, "Check all numerical columns"],
          ...
          "Select Data": {"call": "pb_data.select_data(**parameters)", 
                          'tab': 'Select', 
                                  ...
                                  "after":{"exec":"div=pb_data.full_describe();title='Data Selected';content=format_result(title, '', div, TOOLS, session.get('current_info'));session['current_content']=content;print('>> End')",
                                           "script":"",
        ...
        "Feature Pivot - Dual": {"call": "result = pb_data.feature_summary_dual(**parameters)", 
                                 'tab': 'Feature', 
        ...
        "Distribution Scatter Plot": {"call": "result = pb_data.draw_feature_distribution_scatter(**parameters)", 
                                      'tab': 'Visualization', 
        ...
        "Time Series / Heatmap Plot": {"call": "result = pb_data.draw_feature_time_seires_heatmap(**parameters)", 
                                       'tab': 'Visualization', 
              "info":               'Draw the line plot and heatmap based on the preset time series',
              "available_phase":    [3],
              "type": "ajax", 
              "parameters": {"feature": ["Breakdown Feature", "list(pb_data.classification)", ["choose_eval", "text"], 5, "Select a categorical feature draw its breakdown distribution plot"],
                             "performance": ["Feature to Display", "list(pb_data.performance)", ["choose_eval", "text"], 5, "Select a numeric feature to show its distribution"]
                            },
        ...
        "Custom Feature Relationship Plot": {"call": "result = pb_data.draw_feature_scatter(**parameters);", 
                                             'tab': 'Visualization', 
        ...
        "Generate Topic Model - Plot": {"call": "result = pb_data.generate_topic_model(**parameters)", 
                        'tab':'DA_Models',
        ...
        "Note": {"call": "title, note = pb_data.note(**parameters)", 
                         'tab':"Tools",
              "info":               'Write a comment in this dashboard',
              "available_phase":    [3],
              "type": "ajax", 
              "parameters": {"title": ["Title", "", ["text_edible", 'small', 'text'], 3, "Title of this note"],
                             "note": ["Content", "", ["text_edible", 'large', 'text'], 5, "Content of this note"]
                             },
              "after":{"exec":"""content=format_result(title, '', note, TOOLS, session.get('current_info'));session['current_content']=content;print('>> End')""",
                       "script":"",
                       "finish":"jsonify({'content':content})"
                       }
                },
        
        "Show Details": {"call": "result = pb_data.show_details(**parameters)", 
                         'tab':"Tools",
        ...
        "Reset Data": {"call": "result = '';pb_data.reset();", 
                         'tab':"Tools",
